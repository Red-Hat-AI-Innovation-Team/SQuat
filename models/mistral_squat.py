...
class MistralFlashAttention_SQuat_postrope(nn.Module):
    """
...
class MistralFlashAttention_SQuat(MistralFlashAttention_SQuat_postrope):
    """
    Mistral flash attention module. This module inherits from `MistralAttention` as the weights of the module stays
    untouched. The only required change would be on the forward pass where it needs to correctly call the public API of
    flash attention and deal with padding tokens in case the input contains any of them.
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            if key_states_quant_trans is not None:
                ...
                # att_qkquant = cuda_bmm_fA_qB_outer_rope(self.group_size, query_states, key_states_quant_trans, 
                #                                            key_scale_trans, key_mn_trans, cos, sin, self.k_bits)
                #=========================================================
            else:
                att_qkquant = None
            ...
            key_states_full_repeat = repeat_kv(key_states_full_rot, self.num_key_value_groups)
            att_qkfull = torch.matmul(query_states, key_states_full_repeat.transpose(2, 3))
            ...
            if getattr(self.config, 'squat_query_subspace_prerope', 'after') == 'after':
...
class MistralDecoderLayer_SQuat(nn.Module):
    def __init__(self, config: MistralConfig):
        super().__init__()
        self.hidden_size = config.hidden_size

        if config.method in ["squat", "squat_prerope", "squat_pre"]:
            self.self_attn = MistralFlashAttention_SQuat(config=config)
        elif config.method in ["squat_postrope", "squat_post"]:
            self.self_attn = MistralFlashAttention_SQuat_postrope(config=config)
        else:
...
class MistralModel_SQuat(MistralPreTrainedModel):
    """
    ...
    def __init__(self, config: MistralConfig):
        ...
        self.layers = nn.ModuleList([MistralDecoderLayer_SQuat(config) for _ in range(config.num_hidden_layers)])
...
class MistralForCausalLM_SQuat(MistralPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MistralModel_SQuat(config)
        self.vocab_size = config.vocab_size
