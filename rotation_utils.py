...
from hadamard_utils import apply_exact_had_to_linear_v, apply_exact_had_to_linear_o
...
def my_rotate_ov_proj(layer, head_num, head_dim):
    v_proj = layer.self_attn.v_proj
    o_proj = layer.self_attn.o_proj
    apply_exact_had_to_linear_v(v_proj, had_dim=head_dim)
    apply_exact_had_to_linear_o(o_proj, had_dim=head_dim)


@torch.inference_mode()
def rotate_model_v(model, config):
    num_heads = config.num_attention_heads
    model_dim = config.hidden_size
    head_dim = model_dim // num_heads
    layers = [layer for layer in model.model.layers]
    for idx, layer in enumerate(tqdm.tqdm(layers, unit="layer", desc="Rotating")):
        my_rotate_ov_proj(layers[idx], num_heads, head_dim)
